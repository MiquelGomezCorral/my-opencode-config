...
usage() {
  printf 'Usage: %s [--check] [--skip-deps] [--skip-services] [--home PATH]\n' "$0"
}
...
DEPS_DIR="${XDG_DATA_HOME:-$HOME/.local/share}/my-opencode-config/deps"
...
check_source() {
  ...
  if grep -R -E '/Users/[^/]+|OPENCODE_GO_AUTH_COOKIE|BRAINTRUST_API_KEY' \
...
install_system_dependencies() {
  case "$(uname -s)" in
    ...
    Linux)
      ...
      sudo apt-get update
      sudo apt-get install -y ca-certificates curl git gnupg nodejs npm openssl docker.io sqlite3
...
install_user_dependencies() {
  ...
  opencode upgrade "v$OPENCODE_VERSION"
  bun install -g "btca@$BTCA_VERSION"
...
install_quota() {
  ...
  npm install --prefix "$tools_dir" "pnpm@$PNPM_VERSION"
...
install_config() {
  ...
  local config_entries=(
    ...
    warmup.sh package.json package-lock.json opencode-env.sh agents commands
